...
> **Note:** The package versions used in this example are for illustration purposes only. `@azure/core-http` has since been deprecated and replaced by `@azure/core-rest-pipeline`, and `node-fetch` is no longer a transitive dependency of `@azure/keyvault-keys`. The npm techniques described below apply to any transitive dependency.
